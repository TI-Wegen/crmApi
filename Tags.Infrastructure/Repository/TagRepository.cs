...
public class TagRepository : ITagRepository
{
    ...
    public async Task<Domain.Aggregates.Tags?> GetByNameAsync(string name, CancellationToken cancellationToken = default)
    {
        return await _context.Tags.FirstOrDefaultAsync(x => x.Nome == name, cancellationToken);
    }

    public async Task<PagedResult<Domain.Aggregates.Tags>> GetAllAsync(int pageNumber, int pageSize, CancellationToken cancellationToken = default)
    {
